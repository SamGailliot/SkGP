...
# Sample swiss roll described in paper.
...
# Implementation of Nonparametric Independence Screening procedure (Fan 2014)
...
# Sample white noise sketching matrix
...
# Calculate Hybrid Kernel
hybrid_kernel <- function(S1, S2 = NULL, X1, X2 = NULL, phi = 1, psi = 1, lambda, rho){
  if(is.null(M2)){
    Cs <- plgp::distance(S1)
    Cl <- plgp::distance(X1)
    out <- (phi*exp_kernel(Cs, theta = lambda) + psi*exp_kernel(Cl, theta = rho))
    out <- out / max(out)
    return(out)
  }
  else{
    if (ncol(X1) != ncol(X2)){
      stop("col dim mismatch for X1 & X2. Please ensure data have same dimension")
    }
    Cs <- plgp::distance(S1, S2)
    Cl <- plgp::distance(X1, X2)
    out <- (phi*exp_kernel(Cs, theta = lambda) + psi*exp_kernel(Cl, theta = rho))
    out <- out / max(out)
  }
}
...
get_densities <- function(y, X, m, K, SNRs = c(0.1, 0.5, 1, 2), N.thetas = 50){
  ...
  Density.Mat <- foreach(ii = 1:K, .combine = cbind)%dopar%{
    ...
    for(jj in 1:length(SNRs)){
      ...
      # this is an ugly solution to the problem of tracking and saving both
...
sketched_GP <- function(y, X, y.star, X.star, m = 60, K,
  ...
  if(snr.method == "set"){
    # A matrix that tells me where to look for things, or their values
  ...
  if(stacking.method == "kfold"){
    ...
    for(ii in 1:n.models){
      ...
      # Split out the Densities and Predictions
      loc <- 1
...
# Stacking Weights -- Zhang & Banerjee 2023
